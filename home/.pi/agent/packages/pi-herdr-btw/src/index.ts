...
const AGENT_START_RETRY_INTERVAL_MS = 50;
const AGENT_START_MAX_RETRIES = 20;
		...
		handler: async (args, ctx) => {
			...
			try {
				...
				const launchOptions: HerdrLaunchOptions = {
					...
					activeTools,
					split: route.split ?? config.split,
				...
				let result = await pi.exec("herdr", buildAgentStartArgs(launchOptions, paneId), {
					timeout: 45_000,
				});
				for (let retry = 0; retry < AGENT_START_MAX_RETRIES; retry += 1) {
					if (result.killed || result.code === 0) break;
					const error = safeErrorText(result.stdout, result.stderr);
					if (!error.includes("not an available shell")) break;

					await new Promise((resolve) => setTimeout(resolve, AGENT_START_RETRY_INTERVAL_MS));
					result = await pi.exec("herdr", buildAgentStartArgs(launchOptions, paneId), {
						timeout: 45_000,
					});
				}
