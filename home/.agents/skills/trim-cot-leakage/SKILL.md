...
description: Use when auditing or fixing repository prose that reads like a leaked reasoning transcript — dead design-session citations, change narration, stack or review vantage, reviewer-addressed justifications, control-flow narration, or hedged planning residue in comments, JSDoc, docs, or decision records.
---
...
- **Issue references** — `#1470`, `TODO(name):`, "issue #N owns the follow-up" resolve at HEAD; keep them on any surface, including READMEs. Do not relocate them solely to decision records.
- **Merged-PR and issue citations inside decision records and postmortems** — sanctioned evidence when repository documentation policy assigns change history there.
...
- **Historical stage names inside a decision record's change-story sections** — "the first cut shipped X" is current-state-safe there; indexical stamps ("this cut") stay banned everywhere.
...
1. Scope and exclusions per [prose-standard](../prose-standard/SKILL.md): require an explicit scope; do not edit vendored code, recorded fixtures, snapshots, or sealed history unless the user explicitly targets them.
2. Audit read-only first: run the [recall batteries](references/recall-batteries.md), then judge every hit semantically. The batteries are probes, not the definition, so also read the densest prose in scope without a pattern in hand.
3. Fix owner-first per surface: generated catalogs → fix the source prose or generator template, then regenerate; paired documents → update their counterparts according to repository policy; model-visible strings → wording is behavior, so require the owning behavior check instead of silently rewording.
...
5. Verify: re-run the batteries expecting only sanctioned keeps and this skill's quoted examples; confirm every remaining citation resolves at HEAD; run the repository gates for the touched surfaces.
