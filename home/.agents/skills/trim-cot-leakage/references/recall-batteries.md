...
- Add `--hidden --glob '!.git/**'` when relevant prose lives in dot-directories; ripgrep skips them by default.
- Exclusions go last so a later include cannot re-admit them. Exclude vendored dependencies, generated dependency trees, sealed archives, this skill's own directory, and recorded fixtures or snapshots according to repository layout.
...
- **"This PR" in process docs** — documentation *about* PR workflow ("the PR body should…", templates, repository process notes) legitimately says "PR"; the ban is on a doc adopting one PR's vantage about the code.
