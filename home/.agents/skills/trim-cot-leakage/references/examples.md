...
Use these examples to identify the governing principle, not as text templates. This file deliberately quotes leaked wording as calibration material; its wording is not a license elsewhere.
...
**Fixed:** "Slash input resolves against the visible catalog — the plain-text-reference decision owns this behavior."

The ordinal resolves nowhere at HEAD; the decision's name does. Link its committed owner when one exists.
...
Both live facts survive — the current mechanism and the standing failure behavior — restated in the present. The bug's biography belongs to the PR and its decision record.
...
"This cut" / "v1" / "today" go stale the moment they merge. A historical stage name inside a decision record's change-story section ("the first cut shipped X") is current-state-safe; the indexical form never is.
...
**Fixed (in a decision record's alternatives-considered section):** "**Caching the resolved spec.** Rejected: the spec depends on per-call cwd, so a cache keyed by request would serve stale roots."
...
An unaided pass deleted this, reasoning that issue citations belong in decision records. Wrong direction: issues resolve at HEAD from any surface, and "#N owns the follow-up" is the sanctioned home for deferred work in a README. Decision records and postmortems may additionally cite *merged PRs* as evidence.
...
Every trap below illustrates an overcorrection. Enumerate a passage's propositions before trimming it.
...
**Original:** "The gate notice narrates the check order; the notice text is also compiled by the documentation check."

**Overcorrected:** "…" (whole sentence deleted as narration.)

**Right:** "The notice text is compiled by the documentation check."
