...
- Optional Pi theme and local extensions, generic UI settings and model overrides, plus deliberately pinned third-party Pi packages
...
Pi's package system declares these pinned third-party sources in the linked global `settings.json`:

- `npm:pi-web-access@0.14.0` - web access tools for Pi.
- `npm:pi-herdr-btw@0.3.0` - opens a tool-enabled Pi side thread in a Herdr pane. Its non-sensitive defaults were synchronized from `safe2` into `pi-herdr-btw.json`.
